...
describe("InsightFacade", function () {
    ...
    describe("firstThreeFn", function () {
        ...
        it("should add one correct Dataset", async function () {
            ...
            let res = await insightFacade.addDataset("courses", context, IInsightFacade_1.InsightDatasetKind.Courses);
        ...
        it("should add one Room Datasets", async function () {
            const result = await insightFacade.addDataset("Rooms", roomContext, IInsightFacade_1.InsightDatasetKind.Rooms);
            return (0, chai_1.expect)(result).to.deep.equal(["Rooms"]);
        });
        it("should add multiple Room Datasets", function () {
            return insightFacade.addDataset("Room", roomContext, IInsightFacade_1.InsightDatasetKind.Rooms).then(() => {
                const result = insightFacade.addDataset("Room-2", roomContext, IInsightFacade_1.InsightDatasetKind.Rooms);
                return (0, chai_1.expect)(result).eventually.to.deep.equal(["Room", "Room-2"]);
            });
        });
        it("should add multiple Datasets", function () {
            return insightFacade.addDataset("Courses", context, IInsightFacade_1.InsightDatasetKind.Courses).then(() => {
                const result = insightFacade.addDataset("Room-2", roomContext, IInsightFacade_1.InsightDatasetKind.Rooms);
                return (0, chai_1.expect)(result).eventually.to.deep.equal(["Courses", "Room-2"]);
            });
        });
    ...
    describe("Dynamic Room folder test", function () {
        let insightFacade;
        let content = getFileContent("test/resources/archives/rooms.zip");
        let coursesContent = getFileContent("test/resources/archives/courses.zip");
        before(function () {
            clearDatasets();
            insightFacade = new InsightFacade_1.default();
            return insightFacade.addDataset("courses", coursesContent, IInsightFacade_1.InsightDatasetKind.Courses);
        });
        ...
        (0, folder_test_1.testFolder)("performQuery", (input) => insightFacade.performQuery(input), "./test/resources/queriesSu", {
